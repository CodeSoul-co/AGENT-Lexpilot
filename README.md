...
网页首先展示独立的产品首页，再由用户进入法律自检或专业数据分析工作台。首页采用黑、白、深灰与少量 LexPilot 绿色的法律科技视觉体系，按“产品价值—核心能力—使用场景—安全与隐私—合作案例—行动入口”组织内容，并提供公告栏、固定导航、移动端折叠菜单、克制的滚动淡入和深浅背景切换。首页图片/视频、合作机构、合同分析、案例、数据指标、团队、联系方式和资质信息全部集中在 `web/landing-content.js` 配置；未取得正式材料或尚未开放的能力明确标注“待补充”或“规划能力”，不会以虚构内容代替。媒体配置仅接受同源路径或图片 Data URL，正式资源可放入 `web/` 后修改 `hero.media`、合作 Logo 等字段，无需重写页面结构。法律自检完成后会统一展示“初步自检摘要 + 已核验法条卡片 + 下一步信息准备提示”；无法识别领域时会提供五个支持领域的重新开始入口，避免堆叠互相矛盾的 Agent 状态消息。
