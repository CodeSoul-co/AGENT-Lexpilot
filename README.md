...
基于 [Hypha](https://github.com/CodeSoul-co/Hypha) 框架构建的法律合规业务 Agent，提供两条能力线：

- **V0 法律自检**：面向普通人的免费法律自检工具。用户用自然语言描述遭遇（被辞退、借钱不还、离婚财产分割、侵权盗版、税务纠纷等），Agent 主动追问关键事实，匹配经官方来源核验的法条，输出结构化风险卡片；
- **V1 专业数据查询**：面向法务、律师等专业人员的自然语言数据检索工作站。用户用一句话描述统计需求，Agent 展示只读查询计划，经用户确认后执行，输出表格、图表和可下载的分析文档。
...
- 当前 V1 使用匿名合成的固定演示 Schema 与本地受控求值器，不接真实数据库；Hypha 通用 Text2SQL / SQL Execution 能力到位后再替换。
...
密钥不要写入仓库。真实 Provider 只替换 V0 的结构化事实推理，隐私门、输出校验、法规检索和禁止生成法律结论等边界仍然生效。
...
src/v0/                V0 隐私网关、追问、法规检索、事实比对、加密会话存储
src/v1/                V1 固定演示 Schema 只读查询运行时与执行日志
