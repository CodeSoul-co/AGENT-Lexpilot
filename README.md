...
网页首先展示独立的产品首页，再由用户进入法律自检或专业数据分析工作台。首页采用深绿与暖白的专业服务视觉体系，介绍两项核心能力、四步使用流程、五个首版法律领域及隐私安全边界；尚未提供的合作机构、落地案例、资质荣誉、公开数据、团队与联系方式均以“待补充”占位，不使用虚构信息。法律自检完成后会统一展示“初步自检摘要 + 已核验法条卡片 + 下一步信息准备提示”；无法识别领域时会提供五个支持领域的重新开始入口，避免堆叠互相矛盾的 Agent 状态消息。
