...
执行日志维护必须在应用停止写入的维护窗口内进行。`archive` 先确认删除审计恢复队列为空并验证完整哈希链，再原子写入日志快照和不含本机路径的 SHA-256 清单；归档不会自动删除在线日志。若仍有未对账的删除结果，归档、删除在线副本和删除审计检索都会关闭失败。只有归档再次验证通过、在线日志与归档逐字节一致且操作员输入固定确认短语时，才允许删除在线副本。恢复会再次校验归档内容和哈希链，并且绝不覆盖已经存在的在线日志。归档目录、日志、清单和删除审计恢复队列都属于私有运行数据，已由 `data/` 规则排除在 Git 之外。
...
`find-deletion` 会先验证在线日志和每个归档的清单、字节摘要及哈希链，只返回删除范围、阶段、汇总计数、记录时间和哈希链引用；不会返回伪名 actor、真实会话 ID、Artifact 对象键、正文或本机路径。任何归档篡改、目录中的未知条目、重复/冲突结果或不完整结果链都会使查询整体失败，不会静默跳过。线上策略应把归档目录放在访问受控、具备备份与不可篡改能力的独立存储中，并由组织的数据保留制度确定保存期限。本项目不擅自设定统一法定期限，也不提供自动删除归档的命令。
...
网页首先展示独立的产品首页，再由用户进入法律自检或专业数据分析工作台。首页只把这两项真实业务能力作为产品入口：限定法规检索归入法律自检的辅助能力，计划确认与治理工作流归入专业数据分析的辅助能力，不再展示合同分析等尚未开放的独立产品。首页采用黑、白、深灰与少量 LexPilot 绿色的法律科技视觉体系，按“产品价值—核心能力—使用场景—安全与隐私—合作案例—行动入口”组织内容，并提供公告栏、固定导航、移动端折叠菜单、克制的滚动淡入和深浅背景切换。首页图片/视频、合作机构、公开指标、案例、团队、联系方式和资质信息全部集中在 `web/landing-content.js` 配置；未取得正式材料时统一标注“待补充”，不会以虚构内容代替。媒体配置仅接受同源路径或图片 Data URL，正式资源可放入 `web/` 后修改 `hero.media`、合作 Logo 等字段，无需重写页面结构。法律自检完成后会统一展示“初步自检摘要 + 已核验法条卡片 + 下一步信息准备提示”；无法识别领域时会提供五个支持领域的重新开始入口，避免堆叠互相矛盾的 Agent 状态消息。
...
分析产物默认存入 Git 忽略的 `data/web-demo/v1-artifacts`，也可通过 `LEGAL_V1_ARTIFACT_DIR` 指向其他私有目录。存储对象键由会话、运行和产物标识的 SHA-256 派生，不包含原始标识；网页返回和哈希链日志均不暴露磁盘根路径。归档 Workspace 的历史接口只返回产物元数据与授权下载路径，不返回正文；下载接口再次通过 owner 隔离、归档回执及 Store 内容 SHA-256 校验，且不返回 Store 对象键或根路径。单会话主动删除、账号历史清除和 90 天保留清理都会通过当前 Hypha Store 的公开删除接口校验引用、ETag、内容哈希与大小，删除后再次确认对象不存在；多 Store 操作不宣称分布式事务，Artifact 预检失败时不会先删 Session，发生部分失败时保留 Session 和汇总状态供重试。
