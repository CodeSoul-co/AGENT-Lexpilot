...
const COMPARISON_RULES = Object.freeze(
  Object.fromEntries(
    loadLawCorpus().entries.map((entry) => [
      entry.id,
      Object.freeze({
        legalDomain: entry.legalDomain,
        factRequirements: entry.matching.factRequirements,
        factExclusions: entry.matching.factExclusions,
        excerptSignals: entry.matching.excerptSignals,
        unresolvedElements: entry.matching.unresolvedElements
      })
    ])
  )
);
...
function validateInput(input) {
  ...
  if (!Array.isArray(input.redactedMessages) || input.redactedMessages.some((text) => typeof text !== 'string')) {
...
function baseComparison(input, reference) {
  return {
...
function safeFactCode(field) {
  return field.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`);
}

function compareLaborArticle40(input, reference) {
  const base = baseComparison(input, reference);
  const assessment = assessLaborArticle40Facts({ piiRedacted: true, knownFacts: input.knownFacts });
  const matchedFacts = assessment.requiredFields
    .filter((field) => Object.hasOwn(input.knownFacts, field) && input.knownFacts[field] !== 'unknown')
    ...
    return {
      ...
      unresolvedElements: [
        ...assessment.requiredFields
          ...
          .map((field) => `fact_not_supporting:${safeFactCode(field)}`),
  ...
  if (assessment.status === 'needs_clarification') {
    return {
      ...
      matchedFacts,
      unresolvedElements: [
        ...assessment.missingFields.map((field) => `missing_fact:${safeFactCode(field)}`),
        ...LABOR_ARTICLE_40_UNRESOLVED_ELEMENTS
      ]
    };
  }
...
function compareByRule(input, reference, rule) {
  const base = baseComparison(input, reference);
  ...
  const matchedFacts = observedFacts
    ...
    .map((field) => ({ field, value: input.knownFacts[field] }));
  const excludedFields = Object.entries(rule.factExclusions ?? {})
    .filter(([field, values]) => Object.hasOwn(input.knownFacts, field) && values.includes(input.knownFacts[field]))
    .map(([field]) => field);
  ...
  const sanitizedFactExcerpt = selectSanitizedExcerpt(input.redactedMessages, rule.excerptSignals);
  ...
  return {
    ...
    comparisonStatus: !allRequiredFactsPresent
      ...
      : excludedFields.length > 0 || !allRequiredFactsSupportRule
    ...
    unresolvedElements: [
      ...factFields
        ...
        .map((field) => `missing_fact:${safeFactCode(field)}`),
      ...observedFacts
        .filter((field) => !rule.factRequirements[field].includes(input.knownFacts[field]))
        .map((field) => `fact_not_supporting:${safeFactCode(field)}`),
      ...excludedFields
...
function compareFactsToLaw(input) {
  ...
  const comparisons = input.lawReferences.map((reference) => {
    ...
    if (!rule || rule.legalDomain !== input.legalDomain) {
      return {
        ...baseComparison(input, reference),
        comparisonStatus: 'insufficient_for_comparison',
        sanitizedFactExcerpt: '',
        matchedFacts: [],
    ...
    return compareByRule(input, reference, rule);
  });
    ...
    trace: [
      {
        ...
        data: {
          ...
          potentialMatchCount: comparisons.filter((item) => item.comparisonStatus === 'potential_match').length,
