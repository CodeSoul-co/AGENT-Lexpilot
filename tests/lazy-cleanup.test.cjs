...
function createHarness(options = {}) {
  ...
  let scanCalls = 0;
  const originalScan = store.scanInactive.bind(store);
  store.scanInactive = (inactiveBefore) => {
    scanCalls += 1;
    return originalScan(inactiveBefore);
  };
...
test('startup sweep runs once and entry points are throttled within 24h', () => {
  ...
  assert.equal(harness.scanCalls(), 1);
...
test('long-running process re-runs the sweep on entry points after 24h', () => {
  ...
  harness.service.listHistory();
  assert.equal(harness.scanCalls(), 2);
  ...
  harness.service.answer('missing-session', '测试');
  assert.equal(harness.scanCalls(), 3);
});
