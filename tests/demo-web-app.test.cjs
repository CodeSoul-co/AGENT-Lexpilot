...
test('serves the local web shell and its fixed static assets', async () => {
  await withServer(async (baseUrl) => {
    ...
    assert.match(pageText, /class="app-shell task-panel-collapsed hidden"[^>]*id="workspace-app"/);
    ...
    assert.match(pageText, /class="mode-tab sandbox-mode-tab hidden"[^>]*data-mode="sandbox"/);
    assert.match(pageText, /id="sandbox-language"/);
    assert.match(pageText, /id="sandbox-files"/);
    assert.match(pageText, /class="ghost-button data-source-admin-button hidden"[^>]*id="open-data-source-admin"/);
    assert.match(pageText, /id="data-source-admin-modal"/);
    assert.match(pageText, /id="history-search"/);
    assert.match(pageText, /id="task-panel-toggle"/);
    assert.match(pageText, /id="task-panel"[^>]*aria-label="本次任务"/);
    assert.match(pageText, /id="needs-list"/);
    assert.match(pageText, /id="source-list"/);
    assert.match(pageText, /id="privacy-full-policy"/);
    assert.match(pageText, /class="confirm-summary"/);
    assert.match(pageText, /<summary>高级技术详情<\/summary>/);
    assert.doesNotMatch(pageText, /Agent Runtime|新建 Agent 任务|隐私政策版本/);
    ...
    assert.match(scriptText, /function customerSafeMessage/);
    assert.match(scriptText, /function renderTaskSources/);
    assert.match(scriptText, /function renameHistorySession/);
    assert.match(scriptText, /function deleteHistorySession/);
