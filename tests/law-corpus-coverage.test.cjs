...
test('reports Batch 006 as 36/100 without exposing law text', () => {
  ...
  assert.equal(report.entryCount, 36);
  assert.equal(report.uniqueArticleCount, 36);
  assert.equal(report.duplicateCitationCount, 0);
  assert.equal(report.missingUniqueArticleCount, 64);
  assert.deepEqual(report.missingDomains, []);
  assert.deepEqual(Object.values(report.domainCounts), [8, 7, 7, 7, 7]);
...
test('does not count duplicate citations toward the 100-article gate', () => {
  ...
  assert.equal(report.uniqueArticleCount, 36);
  assert.equal(report.duplicateCitationCount, 64);
  assert.equal(report.missingUniqueArticleCount, 64);
});
...
test('provides a local command that fails honestly at the current 36/100 coverage', () => {
