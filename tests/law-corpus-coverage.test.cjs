...
test('reports Batch 004 as 26/100 without exposing law text', () => {
  ...
  assert.equal(report.entryCount, 26);
  assert.equal(report.uniqueArticleCount, 26);
  assert.equal(report.duplicateCitationCount, 0);
  assert.equal(report.missingUniqueArticleCount, 74);
  assert.deepEqual(report.missingDomains, []);
  assert.deepEqual(Object.values(report.domainCounts), [6, 5, 5, 5, 5]);
...
test('does not count duplicate citations toward the 100-article gate', () => {
  ...
  assert.equal(report.uniqueArticleCount, 26);
  assert.equal(report.duplicateCitationCount, 74);
  assert.equal(report.missingUniqueArticleCount, 74);
});
...
test('provides a local command that fails honestly at the current 26/100 coverage', () => {
