...
test('reports Batch 005 as 31/100 without exposing law text', () => {
  ...
  assert.equal(report.uniqueArticleCount, 31);
  assert.equal(report.duplicateCitationCount, 0);
  assert.equal(report.missingUniqueArticleCount, 69);
  assert.deepEqual(report.missingDomains, []);
  assert.deepEqual(Object.values(report.domainCounts), [7, 6, 6, 6, 6]);
...
test('does not count duplicate citations toward the 100-article gate', () => {
  ...
  assert.equal(report.uniqueArticleCount, 31);
  assert.equal(report.duplicateCitationCount, 69);
  assert.equal(report.missingUniqueArticleCount, 69);
});
...
test('provides a local command that fails honestly at the current 31/100 coverage', () => {
